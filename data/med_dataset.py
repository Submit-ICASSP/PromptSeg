...
class MedDataset(data.Dataset):
    ...
    def __init__(self, 
        ...
        self.task_transform_type = task_transform_type
    ...
    def get_pairs_by_dataset(self, dataset_info, class_name):
        ...
        for i in range(sentence_num):
            ...
            for j in range(self.n_pairs):
                image_info = dataset_info[mode][class_idx]['slice_path'][image_idx[i*self.n_pairs+j]]
                if (not self.random_first) and j == 0:
                    random_now = False
                elif self.random_first and j == 0:
                    random_now = True
                else:
                    random_now = True
                if random_now and self.random_sample_ratio !=0 and random.random() < self.random_sample_ratio:
                    image_info = random.choice(dataset_info[mode][-1]['slice_path'])
    ...
    def __getitem__(self, index):
        ...
        if self.task_transform is not None:
            ...
            if self.task_transform_type == 'all':
                task_transform = tr.Compose(self.task_transform)
            elif self.task_transform_type == 'random':
                task_transform = random.choice(self.task_transform)
            task_data = task_transform(data)
            images = task_data['image']
